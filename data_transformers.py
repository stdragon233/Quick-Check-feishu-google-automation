...
def transform_quickcheck_adaptive(df_raw, sheet_name=None, structure_version="simple", category_version="Quick Check", maps_config=None):
    ...
    融合飞书数据清洗解包安全盾牌，严格恢复原本的字段结构、Excel公式动态列定位与元组完美大小排序。
    """
    if df_raw.empty:
        return pd.DataFrame()
    ...
    for idx, row in df_raw.iterrows():
        ...
        if submission_time is None:
            submission_time = row.get('Submit Time', "")
        ...
        region = clean_feishu_cell_value(row.get("Region", ""))
        country_region = clean_feishu_cell_value(row.get("Country/region", row.get("Country/Region", "")))
        city = clean_feishu_cell_value(row.get("City", ""))
        store_name = clean_feishu_cell_value(row.get("Store Name", ""))
        creator = clean_feishu_cell_value(row.get("Creator", ""))
...
def build_fact_submission(df_raw, form_sheet_name=None):
    ...
    融合飞书数据清洗解包安全盾牌，严格恢复原本的四大板块抽取、通过率百分比格式化及完美的列排序结构。
    """
    if df_raw.empty:
        return pd.DataFrame()
    ...
    if orig_time_col in df_raw.columns:
        # 先统一过一遍安全清洗器，使13位长数字统一变成 "2025-07-02 16:26:57" 字符串
        df_raw[orig_time_col] = df_raw[orig_time_col].apply(clean_feishu_cell_value)
        # 再由 pandas 安全转为标准的 Datetime 对象（这时就能完美认出并转换成功了）
    ...
    for _, r in df_raw.iterrows():
        ...
        creator = clean_feishu_cell_value(r.get("Creator", ""))
        region = clean_feishu_cell_value(r.get("Region", ""))
        country_region = clean_feishu_cell_value(r.get("Country/region", r.get("Country/Region", "")))
        city = clean_feishu_cell_value(r.get("City", ""))
        store_name = clean_feishu_cell_value(r.get("Store Name", ""))
        ...
        row = {
            ...
            "YearMonth": submission_time.strftime("%Y%m") if has_time else "",
            "Quarter": f"{submission_time.year} Q{submission_time.quarter}" if has_time else "",
            "Submission Count": 1
        }
        ...
        for section, cols in section_map.items():
            ...
            pass_cnt = 0 if pd.isna(pass_cnt) else float(pass_cnt)
            nopass_cnt = 0 if pd.isna(nopass_cnt) else float(nopass_cnt)
            na_cnt = 0 if pd.isna(na_cnt) else float(na_cnt)

            total_pass += pass_cnt
            total_nopass += nopass_cnt
            total_na += na_cnt

            row[f"{prefix}_PassRate"] = rate_raw if rate_raw != "" else None
            row[f"{prefix}_PassCount"] = int(pass_cnt)
            row[f"{prefix}_NoPassCount"] = int(nopass_cnt)
            row[f"{prefix}_NACount"] = int(na_cnt)
        ...
        if total_count > 0:
            ...
            row["Overall_PassRate"] = f"{rate_val * 100:.2f}%"
            row["Overall_Performance"] = f"({int(total_pass + total_na)} / {int(total_count)})"
        else:
            row["Overall_PassRate"] = "0.00%"
            row["Overall_Performance"] = "0 / 0"
